...
static struct page *newPage() {
  ...
  stbtt_PackBegin(&page->ctx, &page->textureData[0], PAGE_WIDTH, PAGE_HEIGHT,
                  PAGE_WIDTH, 1, NULL);
...
static int rawRender(struct font *font, struct page *page, struct chardata *cd,
                     uni_t c) {
  ...
  int n = stbtt_PackFontRangesGatherRects(&page->ctx, &font->info, &range, 1,
                                          &rect);
  stbtt_PackFontRangesPackRects(&page->ctx, &rect, n);

  return stbtt_PackFontRangesRenderIntoRects(&page->ctx, &font->info, &range, 1,
                                             &rect);
}
...
static void renderTtfChar(uni_t c, uint8_t attrs, float x, float y) {
  ...
  if (!cd) {
    ...
    glBindTexture(GL_TEXTURE_2D, page->texture);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_ALPHA, PAGE_WIDTH, PAGE_HEIGHT, 0,
                 GL_ALPHA, GL_UNSIGNED_BYTE, &page->textureData[0]);
  }

  stbtt_aligned_quad q;
  stbtt_GetPackedQuad(&cd->packData, PAGE_WIDTH, PAGE_HEIGHT, 0, &x, &y, &q,
                      true);
...
void printChar(const cell_t *cell, float x, float y) {
  ...
  case 0x2089: /* ₉ */
  {
    ...
    glTranslatef(-xx, -yy, 0);
    renderTtfChar(cell->c - 0x2080 + '0', cell->attr, x + fontXOffset,
                  y + fontAscent);
    glPopMatrix();
    break;
  }
